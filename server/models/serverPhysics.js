...
export class ServerPhysics {
  ...
  static resolvePlayerBall(players, ball, onPickup) {
    for (const p of players) {
      ...
      if (!ball.owner) {
        // A stunned player may push the loose ball but cannot immediately take
        // it back after a completed back tackle.
        const canPickup = p.stun <= 0 && (ball.noPickupFrames <= 0 || ball.noPickupFrom !== p.id);
        if (canPickup) {
          ball.x += nx * overlap;
          ball.y += ny * overlap;
          ball.vx += p.vx * 0.22;
  ...
  static updatePlayerPhysics(p, input, ball, onSoundEffect) {
    ...
    if (p.stun > 0) {
      ...
      if (p.passRequestCooldown > 0) p.passRequestCooldown--;
    ...
    if (input.mobileTackleAssist && p.tackleEval > 0) {
      // The assisted lunge owns movement for its brief active window. Keeping
      // the old sideways velocity made a moving player miss despite aiming at
      // the ball, especially on touch devices.
      p.vx = (ax / L) * C.TACKLE_LUNGE;
      p.vy = (ay / L) * C.TACKLE_LUNGE;
    } else {
      // Move toward an explicit target velocity. The old acceleration plus
      // friction formula felt delayed and varied its final speed by frame rate.
      const moving = Math.abs(ax) > 0.001 || Math.abs(ay) > 0.001;
      const targetSpeed = moving ? baseSpeed * 1.2 * dashBoost : 0;
      const response = moving ? 0.22 : 0.18;
      p.vx += (((ax / L) * targetSpeed) - p.vx) * response;
      p.vy += (((ay / L) * targetSpeed) - p.vy) * response;
    }
    ...
    if (p.tackleEval > 0) {
      ...
      // guarantees a back tackle cannot stun both players before the dash ends.
      if (p.tackleEval === 0 && !p.tackleSuccess) {
        p.tackleImpactReady = true;
      }
    }
  }
